...
export class FormLayoutsPage {
    ...
    constructor(page: Page){
        ...
        this.usingTheGridRadioButtons = {
            'Option 1': this.usingTheGridForm.getByRole('radio', { name: 'Option 1' }),
            'Option 2': this.usingTheGridForm.getByRole('radio', { name: 'Option 2' }),
        }
    ...
    async submitInlineFormWithNameEmailAndCheckbox(name: string, email: string, rememberMe: boolean){
        ...
        if(rememberMe)
            await this.inlineFormRememberMe.check({ force: true });
